...
{
  if (!p || !p->CompressedAllele) die ("pbwtWrite called without a valid pbwt") ;
  ...
  long n = arrayMax(p->CompressedAllele) ;
  if (fwrite (&n, sizeof(long), 1, fp) != 1)
    die ("error writing n in pbwtWrite") ;
  if (fwrite ("    ", 1, 4, fp) != 4)
    die ("error writing padding space in pbwtWrite") ;
  if (fwrite (arrp(p->CompressedAllele, 0, uchar), sizeof(uchar), arrayMax(p->CompressedAllele), fp) != arrayMax(p->CompressedAllele))
    die ("error writing data in pbwtWrite") ;

  fprintf (stderr, "written %ld chars pbwt: M, N are %d, %d\n", arrayMax(p->CompressedAllele), p->M, p->N) ;
}
...
void pbwtWriteReverse (PBWT *p, FILE *fp)
{
  ...
  p->CompressedAllele = tz ; p->aFstart = tstart ; p->aFend = tend ;
}
...
PBWT *pbwtRead (FILE *fp) 
{
  ...
  p->CompressedAllele = arrayCreate (nz, uchar) ;
  array(p->CompressedAllele, nz-1, uchar) = 0 ; /* sets arrayMax */
  if (fread (arrp(p->CompressedAllele, 0, uchar), sizeof(uchar), nz, fp) != nz)
...
static PBWT *pbwtReadLineFile (FILE *fp, char* type, ParseLineFunc parseLine)
{
  ...
  while ((*parseLine) (&p, fp, xArray)) /* create p first time round */
    { if (!p->CompressedAllele)		/* first line; p was just made! */
