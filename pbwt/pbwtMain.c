...
static void siteFrequencySpectrum(PBWT *p) {
    ...
    FILE *fp;
    if (p->sites) {
        fp = fopen("sites.freq", "w");
        if (!fp) die("can't open sites.freq");
    }
...
int main(int argc, char *argv[]) {
    ...
    while (argc) {
        if (!(**argv == '-'))
            ...
            FOPEN("writeSamples", "w");
            pbwtWriteSamples(p, fp);
            FCLOSE;
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeMissing") && argc > 1) {
            FOPEN("writeMissing", "w");
            pbwtWriteMissing(p, fp);
            FCLOSE;
            argc -= 2;
            argv += 2;
        }
            ...
            FOPEN("writeMissing", "w");
            pbwtWriteDosage(p, fp);
            FCLOSE;
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeReverse") && argc > 1) {
            FOPEN("writeReverse", "w");
            pbwtWriteReverse(p, fp);
            FCLOSE;
            argc -= 2;
            argv += 2;
        }
            ...
            pbwtWriteImputeRef(p, argv[1]);
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeImputeHapsG") && argc > 1) {
            FOPEN("writeImputeHaps", "w");
            pbwtWriteImputeHapsG(p, fp);
            FCLOSE;
            argc -= 2;
            argv += 2;
        }
            ...
            referenceFasta = strdup(argv[1]);
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeVcf") && argc > 1) {
            pbwtWriteVcf(p, argv[1], referenceFasta, "w");
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeVcfGz") && argc > 1) {
            pbwtWriteVcf(p, argv[1], referenceFasta, "wz");
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeBcf") && argc > 1) {
            pbwtWriteVcf(p, argv[1], referenceFasta, "wbu");
            argc -= 2;
            argv += 2;
        }
        else if (!strcmp(argv[0], "-writeBcfGz") && argc > 1) {
            pbwtWriteVcf(p, argv[1], referenceFasta, "wb");
            argc -= 2;
            argv += 2;
        }
            ...
            p = pbwtSubSampleInterval(p, atoi(argv[1]), atoi(argv[2]));
            argc -= 3;
            argv += 3;
        }
            ...
            p = pbwtSubSites(p, atof(argv[1]), atof(argv[2]));
            argc -= 3;
            argv += 3;
        }
            ...
            p = pbwtRemoveSites(p, sites, FALSE);
            free(chr);
            arrayDestroy(sites);
            argc -= 2;
            argv += 2;
            ...
            p = pbwtSubRange(p, atoi(argv[1]), atoi(argv[2]));
            argc -= 3;
            argv += 3;
        }
        else if (!strcmp(argv[0], "-corruptSites") && argc > 2) {
            p = pbwtCorruptSites(p, atof(argv[1]), atof(argv[2]));
            argc -= 3;
            argv += 3;
        }
        else if (!strcmp(argv[0], "-corruptSamples") && argc > 2) {
            p = pbwtCorruptSamples(p, atof(argv[1]), atof(argv[2]));
            argc -= 3;
            argv += 3;
        }
        else if (!strcmp(argv[0], "-copySamples") && argc > 2) {
            p = pbwtCopySamples(p, atoi(argv[1]), atof(argv[2]));
            argc -= 3;
            argv += 3;
        }
            ...
            FOPEN("siteInfo", "w");
            exportSiteInfo(p, fp, atoi(argv[2]), atoi(argv[3]));
            FCLOSE;
            argc -= 4;
            argv += 4;
        }
        else if (!strcmp(argv[0], "-sfs")) {
            siteFrequencySpectrum(p);
            argc -= 1;
            argv += 1;
        }
        else if (!strcmp(argv[0], "-refFreq") && argc > 1) {
            FOPEN("refFreq", "r");
            pbwtReadRefFreq(p, fp);
            FCLOSE;
            argc -= 2;
            argv += 2;
        }
            ...
            pbwtFitAlphaBeta(p, atoi(argv[1]));
            argc -= 2;
            argv += 2;
        }
            ...
            paintAncestryMatrix(p, argv[1], npr);
            argc -= 2;
            argv += 2;
            if (argc > 0) if (argv[0][0] != '-') {
                --argc;
                ++argv;
            }
        }
            ...
            int npr = 100;
            if (argc > 2) if (argv[2][0] != '-') npr = atoi(argv[2]);
            paintAncestryMatrixSparse(p, argv[1], npr, 0);
            argc -= 2;
            argv += 2;
            if (argc > 0) if (argv[0][0] != '-') {
                --argc;
