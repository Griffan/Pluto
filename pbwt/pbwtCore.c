...
/* "xxx|yyy" where variation is from xxx to yyy in VCF */
...
PBWT *pbwtSubSites(PBWT *pOld, double fmin, double frac) {
    ...
    if (!pOld || !pOld->CompressedAllele) die("subsites without an existing pbwt");
    ...
    pOld->samples = 0;
    pNew->missingOffset = pOld->missingOffset;
    pOld->missingOffset = 0;
    pNew->zMissing = pOld->zMissing;
    pOld->zMissing = 0;
    pbwtDestroy(pOld);
...
PBWT *pbwtSubRange(PBWT *pOld, int start, int end) {
    ...
    if (!pOld || !pOld->CompressedAllele) die("subrange without an existing pbwt");
...
void pbwtBuildReverse(PBWT *p) {
    ...
    if (isCheck)            /* print out the reversed haplotypes */
    {
        ...
        p->CompressedAllele = tz;
        p->aFstart = ta;
    }

    free(x);
    pbwtCursorDestroy(uF);
...
PBWT *pbwtSelectSites(PBWT *pOld, Array sites, BOOL isKeepOld) {
    ...
    fprintf(stderr, "%d sites selected from %d, pbwt size for %d haplotypes is %ld\n",
            pNew->N, pOld->N, pNew->M, arrayMax(pNew->CompressedAllele));
