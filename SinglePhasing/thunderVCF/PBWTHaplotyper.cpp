...
int PBWTHaplotyper::LoopThroughChromosomesViaPBWT() {
    ...
    for (int i = individuals - 1; i >= 0; i--) {
        ...
        PBWTWrapper Graph(2*(individuals-1),markers);
        Graph.setHaps(haplotypes);
                    ...
                    for (int j = 0; j < i; j++) {
                        *output +=
                                *probability * (getTransitionProb(fromWhere, i, k) * getTransitionProb(fromWhere, j, l)
                                                + getTransitionProb(fromWhere, i, l) *
                                                  getTransitionProb(fromWhere, j, k));
                        probability++;
    ...
    for (int j = markers - 2; j >= 0; j--) {
        ...
        for (first = 0; first < states; first++) {
            for (second = 0; second <= first; second++, probability++) {
                sumPerPair = sum+ *probability* getTransitionProb(fromWhere, first, first0) * getTransitionProb(fromWhere, second, second0);
                sum += *probability* (getTransitionProb(fromWhere, first, first0) * getTransitionProb(fromWhere, second, second0)
