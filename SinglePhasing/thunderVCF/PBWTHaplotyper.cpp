...
int PBWTHaplotyper::LoopThroughChromosomesPhasing(Pedigree& ped)
{
    ...
    ResetCrossovers();
    if (isRev and not(runningModel & PHASE))
        ReverseInput();
    ConstructGraph();
    clock_t t1 = clock();
...
{ //ensure this function only called by ITERATIVE
    int begin = 0;
    int end = markers - 1;
...
int PBWTHaplotyper::InitialFwdValues(int sampleIndex, FwdBwdLocalParameter& localParameter)
{
    ...
    absl::flat_hash_map<int, float> dummy;
...
int PBWTHaplotyper::ForwardAlgorithmRec(int sampleIndex, FwdBwdLocalParameter& localParameter, bool greedyMode)
{
    ...
    std::priority_queue<EdgePair, std::vector<EdgePair>,
        std::function<bool(const EdgePair&, const EdgePair&)>>
        EdgePairList(EdgePaircomparator);
    ...
    float alleleCnt(0.f), hapCnt(0.f);// maintain heterozygosity in the ref
    ...
    REENTRY:
        for (auto kv : localParameter.parentsNodeVec[i - 1]) {
            ...
            for (allele1 = 0; allele1 < 2; ++allele1) {
                ...
                for (allele2 = 0; allele2 < 2; ++allele2) {
                    ...
                    {
                        alleleCnt += allele1+allele2;
                        tmpFwdValue = prevFwdValue * GetTransitionProb(i - 1, parentNode1, childNode1) * GetTransitionProb(i - 1, parentNode2, childNode2) * gl;
                        ...
                        fiberCnt += 2;
                        int tmpThinFiberCnt = GetTransitionFreq(i - 1, parentNode1, childNode1) < 3 ? 1 : 0 + GetTransitionFreq(i - 1, parentNode2, childNode2) < 3 ? 1 : 0;
                        if (tmpThinFiberCnt == 2)
                            thinFiberPair = true;
                    ...
                    } else if (reenter) // and gl > 0)
                    {
                        ...
                        tmpFwdValue = prevFwdValue * GetTransitionProb(i - 1, parentNode1, childNode1) * GetTransitionProb(i - 1, parentNode2, childNode2) * gl; //i fwdValueSum
        ...
        //fix singleton
        {
            if ((thinFiberCnt * 2 >= fiberCnt or thinFiberPair or (alleleCnt / hapCnt) < 0.01 or (alleleCnt / hapCnt)> 0.99 ) && !reenter) //if more than 25% singletons
            {
                fitPair = 0;
                localParameter.ClearParentsNodeVec(i);
                localParameter.fwdValueSum[i]=0;//only effective here because insufficient heterozygosity happens even if we observe next level node
                //fprintf(stderr, "[%s] sample %d has insufficient heterozygosity at marker %d\n", __FUNCTION__, sampleIndex, i);
            }
        }
                ...
                for (childNode2 = 0; childNode2 < localParameter.states; ++childNode2) { //dest nodeB
                    ...
                    for (auto tmpParentNode1 : GetParentNodes(i, childNode1)) { //each parent of nodeA
                        for (auto tmpParentNode2 : GetParentNodes(i, childNode2)) { //each parent of nodeB
                            ...
                            baseProb = GetTransitionProb(i - 1, tmpParentNode1, childNode1) * GetTransitionProb(i - 1, tmpParentNode2, childNode2) * gl;
                            ...
                            if (localParameter.fwdValueNode1Sum[i - 1].find(tmpParentNode1) != localParameter.fwdValueNode1Sum[i - 1].end())
                                tmpFwdValue += localParameter.fwdValueNode1Sum[i - 1][tmpParentNode1] * GetHapProbAt(i - 1, tmpParentNode2) * baseProb * (1 - recRate) * recRate;

                            if (localParameter.fwdValueNode2Sum[i - 1].find(tmpParentNode2) != localParameter.fwdValueNode2Sum[i - 1].end())
                                tmpFwdValue += localParameter.fwdValueNode2Sum[i - 1][tmpParentNode2] * GetHapProbAt(i - 1, tmpParentNode1) * baseProb * (1 - recRate) * recRate;

                            tmpFwdValue += GetHapProbAt(i - 1, tmpParentNode2) * GetHapProbAt(i - 1, tmpParentNode1) * baseProb * recRate * recRate;
                            if (DEBUG)
                                fprintf(stderr,
        ...
        for (auto& kv : localParameter.parentsNodeVec[i]) {
            ...
            if (DEBUG)
                for (size_t j = 0; j < localParameter.GetFwdVec(i, kv.second).size(); ++j) {
                    ...
                    fprintf(stderr,
                        "forward marker:%d\tfwdValueSum:%g\tpair:(%d,%d)=(%d,%d)\tcurrentFwd:%g\tnotNormalized currentFwd:%g\tfrom:(%d,%d)\n",
    ...
    for (auto kv : localParameter.parentsNodeVec[markers - 1]) {
        ...
        for (int sourceIndex = 0; sourceIndex < sourceVecSize; ++sourceIndex) { //actually sampling edges
            ...
            if (sum > choice) {
                ...
                sampledFwd = localParameter.GetFwd(markers - 1, destIndex, sourceIndex) * localParameter.fwdValueSum[markers - 1];
...
INIT_SAMPLE_BREAK:
    ...
    for (int i = markers - 2; i > 0; --i) {
        ...
        if (!localParameter.isRec[i]) { //normal hap match without rec
            baseProb = GetTransitionProb(i, sampledParent0, sampledChild0) * GetTransitionProb(i, sampledParent1, sampledChild1) * gl;
...
float PBWTHaplotyper::MatureBwdValue(FwdBwdLocalParameter& localParameter, int site)
{
    ...
    for (auto kv : localParameter.parentsNodeVec[site]) {
        ...
        if(0 && site >654 and site < 702) std::cerr<<site<<"\tsampledChild1:"<<sampledChild1<<"\tsampledChild2:"<<sampledChild2<<"\tnp1:"<<np1<<"\tnp2:"<<np2<<std::endl;
    ...
    int destIndex, float bwdValue, int site)
{
    for (int sourceIndex = 0; sourceIndex < localParameter.GetSourceVec(site, destIndex).size(); ++sourceIndex) {
        ...
        if(0 && site >654 and site < 702) std::cerr<<"\tsampledParent1:"<<sampledParent1<<"\tsampledParent2:"<<sampledParent2<<"\tedgeProb1:"<<edgeProb1<<"\tedgeProb2:"<<edgeProb2<<std::endl;
...
void PBWTHaplotyper::ProcessFwdBwd(int sampleIndex, FwdBwdLocalParameter& localParameter, float sumBaseBwd,
    float* tmpGL, int site)
{
    for (auto kv : localParameter.parentsNodeVec[site]) {
        ...
        if(0 && site >654 and site < 702) std::cerr<<site<<"\tgl:"<<log(gl)<<"("<<(int)allele1<<","<<(int)allele2<<")\tgenotypes[][]:("<<(int)(genotypes[sampleIndex][3*site])<<","<<(int)(genotypes[sampleIndex][3*site+1])<<","<<(int)(genotypes[sampleIndex][3*site+2])<<")";
        ...
        if (site != 0)
            SetNextBwdValue(localParameter, gl, allele1, allele2, destIndex, bwdValue, site);
    }
}
