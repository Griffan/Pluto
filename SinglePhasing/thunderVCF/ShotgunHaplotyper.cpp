...
void ShotgunHaplotyper::WeightByLongestMatch()
{
    ...
    for (int i = 0; i < individuals - 1; ++i) {
        ...
        if (min > nLongestMatch)
            min = nLongestMatch;
        if (max < nLongestMatch)
            max = nLongestMatch;
        sum += nLongestMatch;
        sumsq += (nLongestMatch * nLongestMatch);
    ...
    char phred11, char phred12, char phred22)
{
    ...
    for (int i = 0; i < 3; i++)
        conditional_probs[i] = Penetrance(marker, i, 0) * phred2prob[ph11] + Penetrance(marker, i, 1) * phred2prob[ph12] + Penetrance(marker, i, 2) * phred2prob[ph22];
...
void ShotgunHaplotyper::ImputeAlleles(int marker, int state1, int state2, Random* rand)
{
    ...
    if (r < posterior_11) { //fprintf(stdout,"from 00\t");
        haplotypes[states][marker] = 0;
        haplotypes[states + 1][marker] = 0;
    } else if (r < posterior_11 + posterior_22) { //fprintf(stdout,"from 11\t");
...
bool ShotgunHaplotyper::AllocateMemory(int persons, int maxStates, int m, float defaultTheta)
{
    ...
    if (genotypes == NULL || haplotypes == NULL || marginals == NULL || leftMatrices == NULL || leftProbabilities == NULL || thetas == NULL || crossovers == NULL || error_models == NULL || penetrances == NULL)
...
void ShotgunHaplotyper::SampleChromosomes(Random* rand)
{
    ...
    for (int j = markers - 2; j >= 0; j--) {
        ...
        for (int k = 0; k < states; k++)
            for (int l = 0; l <= k; l++, probability++) {
                ...
                if (second == k || second == l)
                    sum10 += *probability;
                if (first == k && second == l || first == l && second == k)
                    sum00 += *probability;
            }
        ...
        sum = sum11 * theta * theta / (states * states) + (sum10 + sum01) * theta * (1.0 - theta) / states + sum00 * (1.0 - theta) * (1.0 - theta);
        ...
        if (weights != NULL)
            choice /= weights[first / 2] * weights[second / 2];
...
void ShotgunHaplotyper::SelectReferenceSet(int* array, int forWhom, Pedigree& ped)
{
    ...
    if (greedy) {
        ...
        while (chosen < states) {
            ...
            if (array[i] == 1 || i < (individuals - phased))
                continue;
            array[i] = 1;
            chosen += 2;
        }
...
{
    if (DEBUG)
        fprintf(stderr, "now we are looking at individual:%s\n", ped[forWhom].pid.c_str());
    if (DEBUG) {
        int sum_choose(0);
        //int sum_unphased(0);
        for (int i(0); i != individuals - 1; ++i) {
            ...
            if (SHOWLIK)
                fprintf(stderr, "likelihood:%f\t", maxLogLiks[i]);
            if (array[i])
