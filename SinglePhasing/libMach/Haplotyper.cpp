...
//        }
        ...
        if (marginals) delete[] marginals;

        if (leftMatrices) delete[] leftMatrices;
        if (leftProbabilities) delete[] leftProbabilities;

        if (thetas) delete[] thetas;
        if (crossovers) delete[] crossovers;

        if (error_models) delete[] error_models;
        if (penetrances) delete[] penetrances;
        ...
        if (smallMemoryBlock) delete[] smallMemoryBlock;
...
void Haplotyper::LoopThroughChromosomes(ConsensusBuilder &Builder, int SampleTimes, Pedigree &ped,
                                        std::unordered_map<std::string, std::pair<int, int> > &DupTable) {
    bool approximate = (states == individuals * 2 - 2) ? false : true;
        ...
        //int i = 0;
    {
            ...
            //if (phasedSample[i] != -1)//unphased sample
        {
            ...
            int DupIndexPhased = DupTable[std::string(
                    ped[i].pid.c_str())].second;//Duptable:pid,pair(fristIndex,secondIndex)
...
void Haplotyper::IntegrateNPL(float *matrix1, float *matrix2, int marker) {
    ...
    if (weights == NULL)
        for (int disease = 0; disease < diseaseCount; disease++) {
            ...
            matrix1 = matrixStart1;
            matrix2 = matrixStart2;
            for (int i = 0; i < states; i++, matrix1++, matrix2++) {
                int i_status = diseaseStatus[i / 2][disease];
