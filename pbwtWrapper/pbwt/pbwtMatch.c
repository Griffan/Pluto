...
static uchar **checkHapsA;
/* section global for checking only */
static uchar **checkHapsB;
/* section global for checking only */
static int Ncheck;
...
/* algorithm 3 in paper */
{
    ...
    for (k = 0; k <= p->N; ++k)
        for (i = 0; i < u->M; ++i) {
            if (u->d[i] > T) {//the end of last matches interval, then report
                for (ia = 0; ia < na; ++ia)
                    for (ib = 0; ib < nb; ++ib)
...
/* alternative giving start - it turns out in tests that this is also faster, so use it */
{
    ...
    for (k = 0; k <= p->N; ++k) {
        for (i = 0; i < u->M; ++i) {
            if (u->d[i] > k - T) {//current sequence has common sequence with length less than T
...
/* algorithm 4 in paper */
{
    ...
    for (k = 0; k <= p->N; ++k)    // each marker
    {
        for (i = 0; i < u->M; ++i)// each haplotype
        {
            m = i - 1;
...
void pbwtLongMatches(PBWT *p, int L) /* reporting threshold L - if 0 then maximal */
{
    ...
    if (!p || !p->CompressedAllele) die("option -longWithin called without a PBWT");
    if (L < 0) die("L %d for longWithin must be >= 0", L);

    if (isCheck) {
        checkHapsA = checkHapsB = pbwtHaplotypes(p);
        Ncheck = p->N;
    }
    ...
    if (isCheck) {
        int j;
        for (j = 0; j < p->M; ++j) free(checkHapsA[j]);
        free(checkHapsA);
    }
}
...
void matchSequencesNaive(PBWT *p, FILE *fp) /* fp is a pbwt file of sequences to match */
{
    ...
    fprintf(stderr, "Made haplotypes: ");
    timeUpdate();

    if (isCheck) {
        checkHapsA = query;
        checkHapsB = reference;
        Ncheck = p->N;
    }
    ...
    pbwtDestroy(q);
    for (j = 0; j < q->M; ++j) free(query[j]);
    free(query);
    for (j = 0; j < p->M; ++j) free(reference[j]);
    free(reference);
    free(bestSeq);
    free(bestEnd);
}
...
void matchSequencesIndexed(PBWT *p, FILE *fp) {
    ...
    a = myalloc (N + 1, int*);
    for (i = 0; i < N + 1; ++i) a[i] = myalloc (p->M, int);
    d = myalloc (N + 1, int*);
    for (i = 0; i < N + 1; ++i) d[i] = myalloc (p->M + 1, int);
    u = myalloc (N, int*);
    for (i = 0; i < N; ++i) u[i] = myalloc (p->M + 1, int);
    ...
    fprintf(stderr, "Made haplotypes and indices: ");
    timeUpdate();
    ...
    for (j = 0; j < q->M; ++j) {
        ...
        f = 0;
        g = M;
        for (k = 0; k < N; ++k) {               /* use classic FM updates to extend [f,g) interval to next position */
            ...
            else        /* we have reached a maximum - need to report and update e, f*,g* */
            {
                ...
                if ((x[e1] == 0 && f1 > 0) || f1 == M) {
                    f1 = g1 - 1;
                    y = reference[a[k + 1][f1]];
                    while (x[e1 - 1] == y[e1 - 1]) --e1;
...
void matchSequencesSweep(PBWT *p, PBWT *q, void (*report)(int ai, int bi, int start, int end)) {
    ...
    if (isCheck) {
        checkHapsA = pbwtHaplotypes(q);
        checkHapsB = pbwtHaplotypes(p);
        Ncheck = p->N;
    }
    ...
    for (j = 0; j < q->M; ++j) {
        ...
        nTot += (i - f[jj]);
        totLen += (p->N - d[jj]) * (i - f[jj]);
    }

    fprintf(stderr,
            "Average number of best matches including alternates %.1f, Average length %.1f, Av number per position %.1f\n",
...
/* problem that sparse arrays are only progressing at half the rate so k is too big */
{
    ...
    int i;
    for (i = f[j]; i < iPlus; ++i) (*sweepSparseReport)(j, up->a[i], dj, k, isSparse);
    nTot += (iPlus - f[j]);
    totLen += (k - dj) * (iPlus - f[j]);
    ...
    if (isCheck && isSparse)    /* local sparse version of checkMatchMaximal() */
        for (i = f[j]; i < iPlus; ++i) {
            ...
            if (k < Ncheck && x[k] == y[k])
                die("match not maximal - can extend forwards");
            int ii;
            for (ii = dj; ii < k; ii += nSparseStore)
    ...
    if (nSparse > 1)
        for (kk = 0; kk < nSparse; ++kk)
            for (j = 0; j < q->M; ++j) {
                ...
                nTot += (i - ff[kk][jj]);
                totLen += (p->N - dd[kk][jj]) * (i - ff[kk][jj]);
            }
