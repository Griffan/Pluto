...
void imputeExplore(PBWT *p, int test) {
    ...
    typedef struct {
        long n00, n01;
        /* neither neighbour is 1, truth is 0 or 1 */
        long n10a, n10b, n11a, n11b;
        /* one neighbour is 1; a if d is lower for the 0 neighbour */
        long n20, n21;
        /* both neighbours are 1 */
        double fsum;
    } TestStat;
    ...
    if (test == 1)
        ...
        printf("%.3f %.3f\t", cSimple[0] / (double) (cSimple[0] + cSimple[2]),
               cSimple[3] / (double) (cSimple[1] + cSimple[3]));
        printf("%.3f %.3f\t", cCond0[0] / (double) (cCond0[0] + cCond0[2]),
               cCond0[3] / (double) (cCond0[1] + cCond0[3]));
        printf("%.3f %.3f\n", cCond1[0] / (double) (cCond1[0] + cCond1[2]),
               cCond1[3] / (double) (cCond1[1] + cCond1[3]));
    }
    else if (test == 4)
        for (j = 0; j < 17; ++j) {
            ...
            for (i = 0; i < 5; ++i) {
                ...
                xbar += c1[j][i];
                if (i == 3 || i == 4) r2 += c1[j][i];
                if (i == 2) r2 += 0.5 * c1[j][i];
                tot += sum;
            }
...
PBWT *phaseSweep(PBWT *p, PBWT *ref, BOOL isStart, PBWT *r, int nSparse) {
    ...
    PbwtCursor *ur, *uref;
    if (ref) uref = pbwtCursorCreate(ref, TRUE, isStart);// if ref PBWT exists
    if (r) {// if
    ...
    uchar *actualHap = myalloc (M, uchar);  /* actual haplotypes in original order, from p */
    ...
    pbwtCursorDestroy(up);
    pbwtCursorDestroy(uq);
    if (r) pbwtCursorDestroy(ur);
    for (kk = 0; kk < nSparse; ++kk) pbwtCursorDestroy(uqq[kk]);
    free(uqq);
    return q;
}
...
typedef struct {
    TraceBackIndex back : 28;
    /* index into traceBackHeap for cell at previous het site */
...
/* pack this so it fits into a 4-byte word */

typedef struct {
    int j1;
    /* the pair index of j0, which is the index of the cell */
    TraceBackIndex back;
    float s;
#ifdef EXTEND0
  float sBest ;
...
/* returns the number of new back pointers created */
{
    ...
    else if (temp.s > new->s)    /* this is better than previous j that mapped here */
    {
        ...
        *new = temp;    /* originally +=, for Viterbi change to = */
        ...
        {
            new->back = oldNewBack;
            traceBackCell(new->back)->value = x0;
            return 0;
        }
        else {
            traceBackPrune(oldNewBack, k, j0);
...
static PBWT *referencePhase4(PBWT *pOld, PBWT *pRef) {
	...
	{ PhaseCell *pc ; TraceBackCell *tc ;
	  ...
	  putchar ('\n') ; 
        ...
        for (jq = 0; jq < pOld->M; jq += 2) {
            ...
            double sum = 0;
            for (j = 0; j <= pRef->M; ++j) sum += pcNew[jq][j].s;
    ...
    for (jq = 0; jq < pOld->M; jq += 2) {
        ...
        int jMax;
        for (j = 0; j <= pRef->M; ++j)
            if (pcOld[jq][j].s && pcOld[jq][j].s > sMax) {
                sMax = pcOld[jq][j].s;
                jMax = j;
            }
    ...
    pNew->aRstart = myalloc (pNew->M, int);
    for (j = 0; j < pNew->M; ++j) pNew->aRstart[j] = j;
    ...
    pNew->aRend = myalloc (pNew->M, int);
    memcpy (pNew->aRend, uNewR->a, pNew->M * sizeof(int));
    ...
    memcpy (pNew->aFend, uNewF->a, pNew->M * sizeof(int));
    ...
    pbwtCursorDestroy(uNewF);
    pbwtCursorDestroy(uNewR);
    for (jq = 0; jq < pOld->M; jq += 2) {
        free(pcOld[jq]);
        free(pcNew[jq]);
    }
...
PBWT *referencePhase(PBWT *pOld, char *fileNameRoot) {
    ...
    if (!pOld || !pOld->CompressedAllele || !pOld->sites)
    ...
    fprintf(stderr, "Phase preliminaries: ");
    timeUpdate();
    PBWT *pNew = referencePhase4(pOld, pRef);
    fprintf(stderr, "Phasing complete: ");
    timeUpdate();
    fprintf(stderr, "After phasing: ");
    phaseCompare(pNew, pOld);

    pNew->chrom = pOld->chrom;
    pOld->chrom = 0;
    pNew->sites = pOld->sites;
    pOld->sites = 0;
...
static double **pImp = 0;

/* use with stats to store p values */
...
/* Added nSparse to allow also matching at sparse positions */
{
    ...
    while (kRef < pRef->N) {
        ...
        if (pOld == pFrame)    /* find which samples are missing at this site */
        {
            if (!arr(pRef->missingOffset, kRef, long)) bzero(missing, pRef->M);
                ...
                unpack3(arrp(pRef->zMissing, arr(pRef->missingOffset, kRef,
                                long), uchar),
                        pRef->M, missing, 0);
        }
        for (j = 0; j < pOld->M; ++j) {
    ...
    if (nConflicts)
        fprintf(stderr,
                "%d times where no overlapping matches because query does not match any reference - set imputed value to 0\n",
                nConflicts);
    ...
    free(aRefInv);
    free(firstSeg);
    for (j = 0; j < pOld->M; ++j) free(maxMatch[j]);
    free(maxMatch);
    free(xDosage);
    free(yDosage);
...
PBWT *referenceImpute(PBWT *pOld, char *fileNameRoot, int nSparse, double fSparse) {
    ...
    if (!pOld->aFend)
        die("pOld has no aFend in referenceImpute - your pbwt was made by a previous version of the code; buildReverse and resave the forwards pbwt");

    fprintf(stderr, "Imputation preliminaries: ");
    timeUpdate();

    if (isStats) {
        pImp = myalloc (pRef->N, double*);
        int k;
        for (k = 0; k < pRef->N; ++k) pImp[k] = myalloc (pOld->M, double);
    }

    PBWT *pNew = referenceImpute3(pOld, pRef, pFrame, nSparse, fSparse);
    pNew->sites = pRef->sites;
    pRef->sites = 0;
    pNew->chrom = pRef->chrom;
    pRef->chrom = 0;
    pNew->samples = pOld->samples;
    pOld->samples = 0;
    ...
    if (isStats) {
        ...
        for (k = 0; k < pOld->N; ++k)
            if (arr(pOld->missingOffset, k, long)) {
                unpack3(arrp(pOld->zMissing, arr(pOld->missingOffset, k,
                                int), uchar),
                        pOld->M, miss, &n0);
                n0 = pOld->M - n0;
                if (isCheck) {
...
void genotypeCompare(PBWT *p, char *fileNameRoot) {
    ...
    if (!p || !p->CompressedAllele || !p->sites)
...
static void genotypeComparePbwt(PBWT *p, PBWT *q) {
    int i, j, k, ff;
    long n[17][9];
    for (i = 17; i--;) for (j = 9; j--;) n[i][j] = 0;
    double fsum[17];
    for (i = 17; i--;) fsum[i] = 0.0;
    int nsum[17];
    for (i = 17; i--;) nsum[i] = 0;
    double isum[17];
    for (i = 17; i--;) isum[i] = 0.0;
    int ni[17];
    for (i = 17; i--;) ni[i] = 0;
    ...
    for (k = 0; k < p->N; ++k) {
        ...
        if (arrp (p->sites, k, Site)->refFreq) {
            f = arrp (p->sites, k, Site)->refFreq;
            isRefFreq = TRUE;
        }
        for (ff = 0; f * 100 > fBound[ff];) ++ff;
        fsum[ff] += f * 100;
        ++nsum[ff];
        if (arrp(p->sites, k, Site)->imputeInfo < 1.0) {
            isum[ff] += arrp(p->sites, k, Site)->imputeInfo;
            ++ni[ff];
        }
        for (j = 0; j < p->M; ++j) {
        ...
        for (j = 0; j < p->M; j += 2) {
            ...
            if (isDosage) {
                ...
                ++nd[id];
                if (xp[j]) ++nd1[id];
                if (dos[j + 1] == 0.0) id = 0; else if (dos[j + 1] == 1.0) id = 11; else
                    id = 1 + (int) (dos[j + 1] * 10.0);
                ++nd[id];
                if (xp[j + 1]) ++nd1[id];
            }
        }
    ...
    int hist[101];
    for (i = 101; i--;) hist[i] = 0;
    for (j = 0; j < p->M; ++j) {
        double tot = 0, xbar, ybar, r2, x2, y2;
        for (i = 0; i < 9; ++i) tot += ns[9 * j + i];
        if (tot) {
            xbar = (ns[9 * j + 3] + ns[9 * j + 4] + ns[9 * j + 5] +
                    2 * (ns[9 * j + 6] + ns[9 * j + 7] + ns[9 * j + 8])) / tot;
            x2 = (ns[9 * j + 3] + ns[9 * j + 4] + ns[9 * j + 5] + 4 * (ns[9 * j + 6] + ns[9 * j + 7] + ns[9 * j + 8])) /
                 tot;
            ybar = (ns[9 * j + 1] + ns[9 * j + 4] + ns[9 * j + 7] +
                    2 * (ns[9 * j + 2] + ns[9 * j + 5] + ns[9 * j + 8])) / tot;
            y2 = (ns[9 * j + 1] + ns[9 * j + 4] + ns[9 * j + 7] + 4 * (ns[9 * j + 2] + ns[9 * j + 5] + ns[9 * j + 8])) /
                 tot;
...
PBWT *pbwtCorruptSites(PBWT *pOld, double pSite, double pChange) {
    ...
    if (!pOld || !pOld->CompressedAllele) die("corruptSites without an existing pbwt");
...
PBWT *pbwtCopySamples(PBWT *pOld, int Mnew, double meanLength) {
    if (!pOld || !pOld->CompressedAllele) die("copySample called without an existing pbwt");
...
double *pbwtDosageRetrieve(PBWT *p, PbwtCursor *u, double *dosage, int k) {
    ...
    uchar *z = arrp(p->zDosage, arr(p->dosageOffset, k,
            long), uchar);
    int i = 0;
    while (i < p->M) {
        uchar x = *z >> 5;
