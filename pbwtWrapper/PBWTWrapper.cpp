 ...
 */
    ...
    if (k >= 1) {
        for (rank = 0; rank < M; ++rank) {
            if (d[rank] > (k - tmpT)) { //new cluster if current sequence and last sequence have common sequence less than T
                if (rank != 0) {
...
int PBWTWrapper::CopyHap(int k, std::vector<int>& tmpA)
...
bool PBWTWrapper::IsEditDistanceOK(int stateA, int stateB, int index, int error_thresh)
{
    ...
    if (dist[stateA].size() < dist[stateB].size())
    ...
    for (int i = 0; i < sizeB; ++i) { //for each item in smaller set
        ...
        if (lowerRankA == dist[stateA].end())
            lower = backRankB + 1; //if not found, lower bound is rankB + 1
    ...
    if (numTruth == sizeB)
        return true; //if all of the items in smaller set are members of larger set, return true
    return false;
}
    ...
    while (indexA < sizeJ && indexB < sizeK) {
        ...
        if (fabs(Ddiff) > Dmax) {
            ...
            if (sizeJ * sizeK < 10000) {
                if (GetPValue(sizeJ, sizeK, Dmax) < P_thresh)
                    return 1;
            } else {
                if (Dmax > thresh)
                    return 1;
...
int PBWTWrapper::RegressionMergeAtSite(int site)
{
    ...
    for (size_t i = 0; i < dist.size(); ++i) {
        if (HasSiblings(site, i))
            stateWithSibs.push_back(i);
        else
            stateWithoutSibs.push_back(i);
    }
    ...
    while (!mergePairList.empty()) {
        ...
        if (removeIndicator[stateL] || removeIndicator[stateR])
            continue; //skip
        if (retainIndicator[stateL] || retainIndicator[stateR]) { //if either of these two cluster ever merged before, recalculate p value
    ...
    if (ret)
        UpdateCluster(site, removeIndicator);
...
void PBWTWrapper::UpdateCluster(int site, const std::vector<bool>& removeIndicator)
{
    ...
    for (int i = 0; i < (int)clusterMembership.size(); ++i) { //each cluster
        if (clusterMembership[i].size() != 0) {
            ...
            if (prevValidState != -1 and i != 0) {
                ...
                index = site;
                while (index--) {
                    if (haplotype[lastHapID][index] != haplotype[firstHapID][index])
                        break;
                }
                newD = index + 1;
            }
    ...
    float* rate, int phased)
{
    ...
    if (copyEnd == 0)
        return 0; //meaning this was called from reading graph, not need to proceed.
...
int PBWTWrapper::PrintSummary()
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (GetNumStates(i) > maxNodes)
            maxNodes = GetNumStates(i); //clusterAllele[i].size();
    }
